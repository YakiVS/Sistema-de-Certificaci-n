...
function enviar(usu_id, curd_id) {
    // Obtener el teléfono del usuario mediante AJAX
    $.ajax({
        type: "POST",
        url: "../../controller/usuario.php?op=obtener_telefono_usuario",
        data: { usu_id: usu_id },
        dataType: "json",
        success: function (response) {
            if (response.telefono) {
                // Construir el enlace del PDF con el curd_id específico
                var pdfUrl = '*https://eltecnologico.com.pe/Certificados/view/Certificado/index.php?curd_id=' + curd_id + '#loaded*';

                // Construir el mensaje con el enlace
                var mensaje = 'Descarga tu certificado aquí: ' + pdfUrl;

                // Abre la URL de WhatsApp con el número de teléfono y el mensaje
                var whatsappUrl = 'https://wa.me/' + response.telefono + '?text=' + encodeURIComponent(mensaje);
                window.open(whatsappUrl, '_blank');
            } else {
                console.error("No se pudo obtener el teléfono del usuario.");
            }
        },
        error: function () {
            console.error("Error al obtener el teléfono del usuario.");
        }
    });
}
