...
function guardaryeditar(e) {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data === "success") {
                // Si no hay error, recarga la tabla y cierra el modal
                $('#cursos_data').DataTable().ajax.reload();
                $('#modalmantenimiento').modal('hide');
                ...
                Swal.fire({
                    title: 'Error',
                    text: data, // Mostrará el mensaje de error del servidor
                    icon: 'error',
                    confirmButtonText: 'Aceptar'
                });
            }
...
function editar(cur_id){
    $.post("../../controller/curso.php?op=mostrar",{cur_id : cur_id}, function (data) {
        ...
        $('#cur_mes').val(data.cur_mes);
        $('#cur_anio').val(data.cur_anio);
